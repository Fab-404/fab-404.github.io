...
document.addEventListener('DOMContentLoaded', () => {
    console.log("App initialized - Checking EXCEL_DATA...");

    // Check if data loaded
    if (typeof EXCEL_DATA === 'undefined') {
        console.error("EXCEL_DATA is undefined. Check if js/demo-data.js is correctly loaded.");
        document.getElementById('main-content').innerHTML = `
            <div class="flex items-center justify-center h-full text-slate-400">
                <div class="text-center">
                    <i data-lucide="database-zap" class="w-12 h-12 mx-auto mb-3 opacity-50"></i>
                    <p class="font-bold text-slate-800 mb-1">Erreur de chargement</p>
                    <p>Le fichier de données est introuvable ou trop volumineux.</p>
                </div>
            </div>`;
        return;
    }

    console.log("EXCEL_DATA found. Sheets:", Object.keys(EXCEL_DATA));
